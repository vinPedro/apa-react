...
export default function PesquisaPacientePage() {
  ...
  const handleSearch = async () => {
    ...
    try {
      // 1. Defina o endpoint da sua API de busca de histórico resumido.
      const API_ENDPOINT_BUSCA = '/api/prontuarios/search'; 
      
      const response = await fetch(API_ENDPOINT_BUSCA, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
          cpfOrName: searchTerm, 
          startDate: dateStart, 
          endDate: dateEnd 
        }),
      });
      ...
      // 2. Atualize o estado 'results' com os dados resumidos retornados.
  ...
  const handleViewDetails = async (itemResumido) => {
    ...
    try {
      // 1. Defina o endpoint para buscar o prontuário completo pelo ID.
